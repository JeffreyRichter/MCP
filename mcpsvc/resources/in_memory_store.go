...
func (s *InMemoryToolCallStore) Put(_ context.Context, tc *toolcalls.ToolCall, ac svrcore.AccessConditions) error {
	// TODO: reuse the production idempotency code here?
	err := svrcore.ValidatePreconditions(svrcore.ResourceValues{AllowedConditionals: svrcore.AllowedConditionalsNone}, http.MethodPut, ac)
	if err != nil {
		return err
	}
	...
	if _, ok := s.data[key]; ok {
		return svrcore.NewServiceError(http.StatusConflict, "ToolCallIDAlreadyExists", "")
	}
	tc.ETag = svrcore.Ptr(svrcore.ETag(time.Now().Format("20060102150405.000000")))
	s.data[key] = tc.Copy() // storing a copy prevents mutating the caller's data
	return nil
}

...
type localToolCallStore struct {
	data map[string]*toolcall.Resource
	mu   *sync.RWMutex
}
...
func NewToolCallStore(ctx context.Context) toolcall.Store {
	s := &localToolCallStore{data: map[string]*toolcall.Resource{}, mu: &sync.RWMutex{}}
	go s.expiry(ctx)
	return s
}
...
func (s *localToolCallStore) Get(_ context.Context, tc *toolcall.Resource, ac svrcore.AccessConditions) *svrcore.ServerError {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *localToolCallStore) Put(_ context.Context, tc *toolcall.Resource, ac svrcore.AccessConditions) *svrcore.ServerError {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	cp.ETag = new(svrcore.ETag(time.Now().Format("20060102150405.000000")))
	s.data[key] = &cp
	*tc = cp // except we want the caller to have the actual ETag
	return nil
}

func (s *localToolCallStore) Delete(_ context.Context, tc *toolcall.Resource, ac svrcore.AccessConditions) *svrcore.ServerError {
	s.mu.Lock()
	defer s.mu.Unlock()
